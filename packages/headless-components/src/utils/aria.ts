 ...
 * Accepts both AriaPropsConfig and React.AriaAttributes for flexibility
 */
export function generateAriaProps(
  config: AriaPropsConfig | Record<string, any>,
): Record<string, any> {
  const props: Record<string, any> = {};

  // Add role if provided
  if ('role' in config && config.role) {
    props.role = config.role;
  }

  // Add aria-disabled based on disabled state
  if ('disabled' in config && config.disabled !== undefined) {
