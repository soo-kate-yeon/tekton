 ...
 */
export interface PresetClientConfig {
  baseUrl: string;
  timeout: number;
}
...
export class PresetClient {
  private config: PresetClientConfig;

  constructor(config?: Partial<PresetClientConfig>) {
    this.config = {
   ...
   */
  async listPresets(params: PresetListParams = {}): Promise<PresetListResponse> {
    ...
    return this.request<PresetListResponse>(endpoint, { method: 'GET' });
  }

  /**
   * Get a preset by ID
   */
  async getPreset(id: number): Promise<CuratedPreset> {
    return this.request<CuratedPreset>(`/api/v2/presets/${id}`, { method: 'GET' });
  }
   ...
   */
  async getPresetByName(name: string): Promise<CuratedPreset | null> {
    try {
   ...
   */
  async getPresetsByCategory(category: string): Promise<CuratedPreset[]> {
   ...
   */
  async getPresetSuggestions(context?: string): Promise<CuratedPreset[]> {
    try {
      const endpoint = context
        ? `/api/v2/presets/suggestions?context=${encodeURIComponent(context)}`
        : '/api/v2/presets/suggestions';

      return this.request<CuratedPreset[]>(endpoint, { method: 'GET' });
    } catch {
 ...
 */
export const presetClient = new PresetClient();
