...
const NavItem = ({ children, active }: { children: React.ReactNode; active?: boolean }) => (
    <button className={`flex flex-col items-center px-3 py-1 rounded-[--lm-radius-md] transition-all duration-[--lm-motion-duration-moderate] ${active ? 'text-[--lm-brand-emphasis]' : 'text-[--lm-text-tertiary] hover:text-[--lm-text-secondary]'}`}>
        <span className="text-[11px] font-[--lm-font-weight-bold] tracking-tight">{children}</span>
        {active && <div className="mt-1 w-1 h-1 rounded-full bg-[--lm-brand-default] shadow-[0_0_8px_var(--lm-brand-500)]"></div>}
    </button>
);
